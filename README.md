...
Content script for the bridge runs on `localhost`, `127.0.0.1`, and the
production origin `cashepos.com` (`https://cashepos.com` and
`https://www.cashepos.com`). If the app is served from another origin, add it
to `host_permissions` and to the `content_scripts[0].matches` entry in
`manifest.json`.
